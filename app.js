...
document.getElementById("saveMasterBtn").onclick = async function() {
    ...
    masterProducts.push(newProdPayload);
    try { await db.from('products_master').insert([newProdPayload]); } catch(e){ console.error(e); }
    ...
    if(qty > 0) {
        ...
        try { await db.from('inventory_batches').insert([newBatch]); } catch(e){ console.error(e); }
    }
